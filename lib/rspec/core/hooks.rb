module RSpec
  module Core
    ...
    module Hooks
      ...
      # after `before` context hooks but before any `before` example hook
      # regardless of scope.
      #
      # ### Conditions
      #
      ...
      # `around` hooks will run after any `after` example hooks are invoked but
      # before any `after` context hooks.
      #
      # @note The `:example` and `:context` scopes are also available as
      #       `:each` and `:all`, respectively. Use whichever you prefer.
      # @note The `:suite` scope is only supported for hooks registered on
